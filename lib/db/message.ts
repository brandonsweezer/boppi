...
export class MongoDBMessageRepository implements MessageRepository {
    ...
    async readByType({ messageType }: { messageType: MessageType; }): Promise<Message[]> {
        try {
            await this.client.connect();
            const message = await this.client.db(APP_DB_NAME).collection(MESSAGE_COLLECTION_NAME).find(
                {
                    type: messageType,
                }
            )
